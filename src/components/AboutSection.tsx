  ...
  const features = [
    {
      ...
      description: "State-of-the-art fitness machines and equipment to ensure effective workouts.",
      icon: Dumbbell
    },
    {
      title: "Expert Trainers",
      description: "Certified personal trainers who provide customized fitness programs.",
      icon: Users
    },
    {
      title: "Diverse Classes",
      description: "Various classes ranging from HIIT to yoga for all fitness levels.",
      icon: Activity
    },
    {
  ...
  return (
    <section id="about" className="relative py-24 bg-transparent overflow-hidden">
      ...
      <div className="container mx-auto px-4 z-10 relative">
        ...
        <div className={`grid grid-cols-2 md:grid-cols-4 gap-4 mb-16 transition-all duration-1000 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          {achievements.map((item, index) => (
            <div 
              key={index} 
              className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-lg p-6 text-center transition-all hover:bg-white/20"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className={`lg:col-span-1 transition-all duration-1000 ${isVisible ? 'translate-x-0 opacity-100' : '-translate-x-12 opacity-0'}`}>
            <div className="relative pl-8 border-l border-white/20 space-y-8">
              {/* Timeline items */}
              {[2008, 2012, 2018, 2023].map((year, index) => (
                <div key={index} className="relative" style={{ transitionDelay: `${index * 150}ms` }}>
                  ...
                  <div className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-lg p-4">
                    ...
                    <p className="text-gray-300 text-sm">
                      {index === 0 && "Apex Fitness was founded with a mission to transform lives"}
                      {index === 1 && "Expanded to include specialized training programs"}
                      {index === 2 && "Opened 5 new locations across the country"}
                      {index === 3 && "Introduced holistic wellness approach"}
                    </p>
                  </div>
                </div>
              ))}
          ...
          <div className={`lg:col-span-1 transition-all duration-1000 delay-300 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
          ...
          <div className={`lg:col-span-1 transition-all duration-1000 delay-500 ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-12 opacity-0'}`}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {features.map((feature, index) => (
                <div 
                  key={index} 
                  className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-lg p-4 hover:bg-white/20 transition-all duration-300"
        ...
        {/* Image Gallery */}
        <div className={`mt-16 transition-all duration-1000 delay-700 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative">
            <div className="md:col-span-1 aspect-video md:aspect-square rounded-xl overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-t from-gray-900/80 to-transparent z-10"></div>
              <img 
                src="https://images.unsplash.com/photo-1581009146145-b5ef050c2e1e?ixlib=rb-1.2.1&auto=format&fit=crop&w=1050&q=80"
                alt="Gym training"
