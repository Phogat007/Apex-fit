...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['home', 'about', 'classes', 'trainers', 'pricing'];
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className={`absolute bottom-0 left-0 right-0 h-[1px] ${
          isScrolled ? 'bg-gradient-to-r from-transparent via-white/30 to-transparent' : 'bg-transparent'
        }`}>
      ...
      <nav className="container mx-auto px-4 flex items-center justify-between relative z-10">
        <div className="flex items-center">
          <a href="/" className="group flex items-center">
            <div className="mr-2">
              <Activity className={`w-6 h-6 ${isScrolled ? 'text-white' : 'text-white'} transform transition-all group-hover:scale-110 duration-300`} />
            </div>
        ...
        <div className="hidden md:flex items-center space-x-8">
          {navLinks.map((link) => (
            <a 
              key={link.href}
              href={link.href}
              onClick={e => handleSectionNav(e, link.href.substring(1))}
              className={`text-gray-200 relative group ${
                activeSection === link.href.substring(1) ? 'text-white' : 'text-gray-300'
              }`}
            >
              {link.label}
              <span className={`absolute -bottom-1 left-0 w-full h-0.5 bg-gradient-to-r from-white to-gray-400 transform origin-left transition-transform duration-300 rounded-full ${
          ...
          <Button
            className={`bg-gradient-to-r from-white to-gray-400 hover:from-gray-200 hover:to-gray-500 text-black transition-all group duration-300 shadow-lg shadow-white/10 hover:shadow-white/20 ${
          ...
          <div className="flex flex-col px-4 py-6">
            {navLinks.map((link, index) => (
              <a 
                ...
                href={link.href}
                onClick={e => handleSectionNav(e, link.href.substring(1))}
                className={`text-gray-200 py-3 relative transition-all transform ${
                  activeSection === link.href.substring(1) ? 'text-white pl-4' : 'text-gray-300 hover:pl-4'
                } animate-fade-slide-down`}
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full ${
                  activeSection === link.href.substring(1) ? 'bg-white' : 'bg-transparent'
                }`}></span>
                {link.label}
              </a>
            ))}
            {additionalLinks.map((link, index) => (
              <a 
                key={link.href}
                href={link.href} 
                className="text-gray-200 py-3 relative transition-all transform hover:pl-4 animate-fade-slide-down"
                style={{ animationDelay: `${(index + navLinks.length) * 100}ms` }}
                onClick={() => setIsMobileMenuOpen(false)}
              >
                <span className="absolute left-0 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-transparent"></span>
                {link.label}
              </a>
            ))}
            ...
            <div className="mt-6 bg-white/5 p-4 rounded-lg backdrop-blur-sm border border-white/10 animate-fade-slide-down" style={{ animationDelay: '500ms' }}>
              <div className="flex flex-col space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-200 text-sm">Join 10,000+ active members</span>
                </div>
                <div className="flex items-center gap-2">
                  <Dumbbell className="text-white w-4 h-4" />
                  <span className="text-gray-200 text-sm">Access to 200+ equipment</span>
                </div>
                <Button 
                  className="bg-gradient-to-r from-white to-gray-400 hover:from-gray-200 hover:to-gray-500 text-black w-full shadow-lg shadow-white/10"
                  onClick={() => {
                    navigate('/trial');
                    setIsMobileMenuOpen(false);
                  }}
