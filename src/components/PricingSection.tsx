      ...
      <div className="container mx-auto px-4 z-10 py-16 lg:py-0">
        <div className={`text-center mb-16 transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
          ...
          <div className="flex items-center justify-center space-x-4 mt-8">
            ...
            <button
              ...
              style={{ backgroundColor: isAnnual ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.1)' }}
            >
              <span
            ...
            <span className={`text-sm ${isAnnual ? 'text-gray-400' : 'text-white'}`}>
              Annual <span className="text-white/60">(Save 20%)</span>
            </span>
          </div>
        </div>
        ...
        <div className={`grid md:grid-cols-3 gap-8 transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
          {plans.map((plan, index) => (
            <div
              key={plan.name}
              className={`relative bg-white/10 backdrop-blur-sm rounded-2xl overflow-hidden border border-white/10 hover:bg-white/20 transition-all duration-300 ${
                plan.popular ? 'md:-mt-8 md:mb-8' : ''
              }`}
            >
              {plan.popular && (
                <div className="absolute top-0 right-0 bg-gradient-to-r from-white/20 to-white/10 px-4 py-1 rounded-bl-lg backdrop-blur-sm">
